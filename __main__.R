...
# Specify interaction strength. Note: When Intr[1]==Intr[3] no interaction between C1-A and I;
# When Intr[2]==Intr[4] no interaction between C2-A and I.
# Interaction terms explained: 
# Intr[1]: Pr(A|C1=c,I=0)/Pr(A|C1=c-1,I=0), Intr[2]: Pr(A|C2=c,I=0)/Pr(A|C2=c-1,I=0);
# Intr[3]: Pr(A|C1=c,I=1)/Pr(A|C1=c-1,I=1), Intr[4]: Pr(A|C2=c,I=1)/Pr(A|C2=c-1,I=1).
Intr = c(1.35,1.15,1.25,1.185)

# Specify position and var of Y. By definition only is y normally distributed 
#  will our model be free of mis-specification.
mean_Y = 5 # E(Y|C1=C2=Intr[]=A=0)
var_Y = 2.55

# Specify Effects
A_prob = 0.19 # Pr(A|C1=C2=Intr[]=V=0); 
I_prob = 0.475 # Marginal Pr(I=1)
E_A_Y = 2.25 # True treatment effect
E_C1_Y = 2.05 # Effect of C1 on Y
E_C2_Y = 1.35 # Effect of C2 on Y
RR_V_A = 0.79 # Effect of V on A.
...
call <- expr(
  simulationInteraction(sim_runs = sim_runs,size = size,distr_C1 = distr_C1,
                       center_C1 = center_C1,var_C1 = var_C1,min_C1 = min_C1,
                       max_C1 = max_C1,distr_C2 = distr_C2,center_C2 = center_C2,
                       var_C2 = var_C2, min_C2 = min_C2,max_C2 = max_C2,
                       distr_V = distr_V,center_V = center_V,var_V = var_V,
                       min_V = min_V,max_V = max_V,Intr = Intr,mean_Y = mean_Y,
                       var_Y = var_Y,A_prob = A_prob,I_prob = I_prob,
                       E_A_Y = E_A_Y,E_C1_Y = E_C1_Y,E_C2_Y = E_C2_Y,RR_V_A = RR_V_A,
                       formula_list = formula_list) # argument added!
) # Note: ... not included here. If one want to use other formula list please modify this call.
